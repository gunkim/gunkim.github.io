...
Compaction을 원활히 수행하기 위해 Handle과 같은 자료구조를 사용할 수도 있다. (Root Set에서 Reference되고 있는 Live Object의 Handle 주소에 flag값을 마킹)
...
Train 알고리즘은 Heap을 Train으로 구성한다. 이 Train은 Car(객차)라 불리는 Fixed Size의 Memory Block을 묶어놓은 체인이다. 필요에 따라 Train이 Car를 추가할 수도, 하나의 Train 자체가 추가될 수도 있다.
