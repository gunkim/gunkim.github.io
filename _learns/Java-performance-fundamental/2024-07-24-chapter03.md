...
가비지 컬렉션은 프로그램 실행 중에 사용되지 않는 메모리를 자동으로 회수하는 기능이다. 이를 통해 개발자는 메모리 관리의 부담을 줄이고, 메모리 누수(memory leak)를 방지할 수 있다. 그러나 가비지 컬렉션이 실행되는 동안 프로그램의 실행이 잠시 중단되기도 하며, 이는 CPU 사용이 민감한 프로그램의 경우 성능에 영향을 줄 수 있다.

JVM 각 벤더는 여러 Garbage Collector를 제공하기 때문에 어떤 Garbage Collector를 선택하느냐도 성능에 있어서 중요한 이슈가 된다.
...
![Garbage Collection - Reachable and Garbage Object](/public/img/learn/java-performance-fundamental/03-1.png)
...
![Garbage Collection - Heap Fragmentation](/public/img/learn/java-performance-fundamental/03-2.png)
...
Garbage Object를 찾아내는 감지(Detection)에 초점이 맞춰진 알고리즘이다.

초기 알고리즘답게 각 Object마다 Reference Count를 관리하여 Reference Count가 0이 되면 GC를 수행하도록 단순하게 구성되어 있다.

![Reference Counting Algorithm](/public/img/learn/java-performance-fundamental/03-3.png)

그렇기 때문에 정지 시간(Pause Time)이 분산되어 실시간 작업에도 거의 영향을 주지 않는 장점이 있다.

하지만 Reference가 변경될 때마다 각 Object의 Reference Count를 변경해 주어야 하기 때문에 관리 비용이 상당하다. 또한, GC가 연쇄적으로 일어날 수 있다는 것도 문제가 될 수 있다.
...
이때 GC는 표시(Mark)와 정리(Sweep) 두 단계에 걸쳐 일어난다.

Mark 단계는 Garbage Object를 구별하는 단계로 Root Set에서 Reference 관계가 있는 Object에 Marking하는 방식으로 수행된다. Marking하기 위해 주로 Object Header에 Flag나 별도의 Bitmap Table 등을 사용한다.

![Mark-and-Sweep Algorithm - Mark Phase](/public/img/learn/java-performance-fundamental/03-4.png)

Mark 단계가 끝나면 즉시 Sweep 단계로 돌입한다. 이 단계는 Marking되지 않은 Object를 지우는 작업을 한다.

![Mark-and-Sweep Algorithm - Sweep Phase](/public/img/learn/java-performance-fundamental/03-5.png)

Mark-and-Sweep 알고리즘은 Reference 관계가 정확히 파악되고 Reference 관계 변경 시 부가적인 작업을 하지 않기 때문에 속도가 향상된다는 장점이 있다. 하지만 Mark 작업의 정확성과 메모리 손상(Memory Corruption)을 방지하기 위해 중단되는 현상이 발생한다.
...
![Mark-and-Compaction Algorithm - Marking Phase](/public/img/learn/java-performance-fundamental/03-6.png)
...
![Mark-and-Compaction Algorithm - Compaction Phase](/public/img/learn/java-performance-fundamental/03-7.png)

Compaction Phase에서는 일단 Marking된 정보를 바탕으로 Garbage Object를 Sweep한 후 Heap의 한쪽 방향으로 Live Object를 이동시킨다. 이때 Sliding Compaction 방법을 사용한다.

Mark-and-Compaction 알고리즘은 단편화 방지에 초점이 맞춰져 있기에 메모리 공간 효율성이 가장 큰 장점이 된다. Compaction 이후에 모든 Reference를 업데이트하는 작업은 경우에 따라 모든 Object를 Access하는 등 부가적인 오버헤드를 수반할 수 있다. 또한 Mark Phase와 Compaction Phase는 모두 중단 현상이 발생한다는 단점도 있다.

#### Copying 알고리즘

단편화 문제를 해결하기 위해 제시된 또 다른 방법이다.

Heap을 Active 영역과 Inactive 영역으로 나누어 사용하는 특징을 가진다. 이 중 Active 영역에만 Object를 할당받을 수 있고 Active 영역이 꽉 차게 되어 더 이상 Allocation이 불가능하게 되면 GC가 수행된다.

GC가 수행되면 모든 프로그램은 일단 중단 상태가 된다. 그리고 Live Object를 Inactive 영역으로 Copy하는 작업을 수행한다. 이 알고리즘을 Stop-the-Copying이라고도 부르는 이유이다. Object를 Copy할 때 각각의 Reference 정보도 같이 변경된다.

이렇게 GC가 수행될 경우 Active 영역에는 Garbage Object만, Inactive 영역에는 Live Object만 남게 된다. 이때 Inactive 영역에 Object를 Copy할 때 한쪽 방향에서부터 차곡차곡 적재하기 때문에 마치 Compaction된 것처럼 정렬된다.

![Copying Algorithm](/public/img/learn/java-performance-fundamental/03-8.png)
...
Copying 알고리즘은 단편화 방지에 효과적이지만 전체 Heap의 절반 정도를 사용하지 못한다는 단점이 존재하며, 중단 현상과 Copy 오버헤드는 필요악이라 할 수 있다.
...
![Generational Algorithm](/public/img/learn/java-performance-fundamental/03-9.png)
...
Tracing 알고리즘이 등장한 후 GC를 수행할 때 프로그램에 중단 현상이 나타나는 것은 감수할 수밖에 없었다. 오히려 GC는 메모리를 재사용하기 위한 것이기에 목적에 충실했었을 것이라 추측된다.

하지만 전반적인 성능에서 GC 시 발생하는 중단은 좋지 않다. WAS처럼 짧은 트랜잭션을 처리하는 시스템에서 중단 현상은 사용자에게 불쾌감을 주어 비즈니스에 악영향을 끼친다.

Train 알고리즘은 이러한 배경에서 나온 듯하다. Train 알고리즘은 Heap을 작은 Memory Block으로 나누어 Single Block 단위로 Mark Phase와 Copy Phase로 구성된 GC를 수행한다. 이러한 특징 때문에 Incremental 알고리즘이라고도 한다.

GC 단위가 Single Block인 만큼 전체 Heap의 중단이 아닌 GC 중인 Memory Block만 중단이 발생한다. 다시 말해 중단을 분산시켜 전체적인 Pause Time을 줄이자는 아이디어이다.

![Train Algorithm](/public/img/learn/java-performance-fundamental/03-10.png)
...
![Hotspot JVM Generation Structure](/public/img/learn/java-performance-fundamental/03-11.png)
...
![Minor GC and Major GC](/public/img/learn/java-performance-fundamental/03-13.png)
...
![Write Barrier and Card Table](/public/img/learn/java-performance-fundamental/03-12.png)
...
<details>
  ...
  <ul>
    ...
    <li><code>-Xmx&lt;size&gt;</code>: 확장 가능한 Heap의 최대
...
Young & Old Generation에서 GC를 Single CPU(1 Thread)를 통해 처리한다.
...
![Serial Collector - Young Generation](/public/img/learn/java-performance-fundamental/03-14.png)
...
![Promotion Process During Minor GC](/public/img/learn/java-performance-fundamental/03-15.png)

**Old Generation : Mark-and-Compacting 알고리즘**

Old Generation은 Young Generation과 다른 방식으로 GC를 수행한다. Old Generation의 크기가 Young Generation보다 크고 수행 방식에 차이가 있기 때문에 Minor GC보다 정지 시간이 길다. 이때 Garbage를 정리(Sweep), 정렬(Compaction) 과정에서 시간을 많이 소요한다.

게다가 Minor GC 와중에 Promotion을 위한 Old Generation에서의 공간이 부족하게 되면 Full GC가 이어서 일어나기 때문이다.

![Serial Collector - Old Generation](/public/img/learn/java-performance-fundamental/03-16.png)

### Incremental Collector

낮은 일시 정지 목표(Low Pause Goal)를 가진 최초의 Collector이다. Low Pause 전략은 GC로 인한 정지 시간을 분산시키는 것이 핵심이다.
...
Single Block 단위로 GC를 수행하기 때문에 정지 시간이 분산되어 Low Pause가 된다.
...
![Incremental Collector](/public/img/learn/java-performance-fundamental/03-17.png)
...
Copy 작업에서 중단 현상이 발생하는 건 동일하지만 리소스를 더 투입하는 만큼 시간이 단축된다.

![Parallel Collector - Young Generation](/public/img/learn/java-performance-fundamental/03-18.png)
...
![Parallel Collector - Promotion Buffer](/public/img/learn/java-performance-fundamental/03-19.png)
...
CMS Collector는 Fast Elapsed Time에 중점을 두고 있어 Low Latency Collector라고도 불린다. GC에서 수반되는 정지 시간을 적절히 분산하여 응답 시간을 개선하는 방식을 사용한다.
...
1. **Initial Mark 단계**는 Serial 단계이며 중단 상태가 된다. 그러나 Root Set에서 직접 Reference(1 Depth)되는 Object만을 대상으로 하기 때문에 중단 시간은 최소화된다.
2. **Concurrent Mark 단계**도 Serial 단계이며 Initial Mark 단계에서 선별된 Live Object를 대상으로 Reference하고 있는 Object를 추적하여 Live 여부를 구별한다.
  - Concurrent가 이름에 들어가는 이유는 다른 JVM 내 Working Thread들은 그대로 동작한다. (즉 중단되지 않음)
3. **Remark 단계**는 Parallel 단계로 모든 Thread가 GC에 동원된다. 그렇기 때문에 중단 시간이 발생하며 이미 Marking된 Object를 다시 추적하여 Live 여부를 확정하게 된다. Remark 단계가 가장 작업량이 많기 때문에 가용한 모든 리소스를 투입하는 것이다.
...
![CMS Collector - Old Generation](/public/img/learn/java-performance-fundamental/03-20.png)

Compaction을 수행하기 위해서는 Heap의 중단이 전제되어야 한다. 그러나 CMS Collector는 Compaction을 포기하고 최대한 Application의 수행을 보장한다.
...
![CMS Collector - Old Generation Sweep Phase](/public/img/learn/java-performance-fundamental/03-21.png)
...
CMS Collector는 Major GC에서 중단 시간을 최대한 감소시키는 방식으로 동작하지만 정지 시간을 완벽히 없애지는 못했다. 그리고 Minor GC와 Major GC에서 Remark가 연달아 수행된다면 중단 시간 목표에서 의도한 정지 시간보다 길어질 것이다.

이를 위해 Schedule을 고려하며 두 가지 목적이 존재한다.

1. Minor GC와 Remark Phase가 연달아 수행되지 않도록 방지함.
...
- Scheduling을 통해 Minor GC와 Full GC가 동시에 발생하지 않도록 조정하며, Initial Mark Phase는 빠르게 완료되어 별도의 Scheduling에서 제외된다.

**Incremental Mode of CMS Collector**

CMS Collector에서 Concurrent 작업이 큰 실효성을 거두지 못할 경우 Pause Time을 줄이는 효과가 나타나지 않는다.

![Incremental Mode of CMS Collector](/public/img/learn/java-performance-fundamental/03-22.png)
...
![](/public/img/learn/java-performance-fundamental/03-23.png)

Mark 단계는 Old Generation을 Region이라는 단위로 나누어 병렬 작업으로 수행하는 단계다. 각 Region에서 Live Object를 표시하고, 이 과정에서 객체 크기와 위치 정보가 갱신된다.

![](/public/img/learn/java-performance-fundamental/03-24.png)

Summary 단계에서는 단일 스레드가 GC를 수행하며, 나머지 스레드는 애플리케이션을 실행한다. 이 단계에서는 Region 단위로 밀도(Density)를 평가해 Dense Prefix를 설정한다. Dense Prefix는 도달 가능한 객체가 많은 Region을 구분하는 것으로, 이후 Compaction Phase의 대상을 결정한다. 이로 인해 Compaction 범위가 줄어들어 Garbage Collection 시간이 단축된다.

![](/public/img/learn/java-performance-fundamental/03-25.png)
...
![](/public/img/learn/java-performance-fundamental/03-26.png)
...
![](/public/img/learn/java-performance-fundamental/03-27.png)
...
![](/public/img/learn/java-performance-fundamental/03-28.png)
...
![](/public/img/learn/java-performance-fundamental/03-29.png)
...
![](/public/img/learn/java-performance-fundamental/03-30.png)
